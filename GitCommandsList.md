...
*git commit -m .gitignore* делает коммит только для одного modified файла .gitignore и не затрагивает остальные. При этом действует тоже ограничение что и для -a: файлы которые Git не отслеживает - будут проигнорированны. Иными словами если на момент коммита файл .gitignore
untracked - коммит не пройдёт.
