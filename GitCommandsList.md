...
*git commit -a* (Далее запускается текстовый редактор и делаем описание коммита)
Можно добавить ключ -m и сделать краткое пояснение к коммиту без вызова редактора
*git commit -am "Залил фотку Тефтельки"*

**Важно понимать что хоть ключ -a и позволяет пропустить добавление в индекс он работает только с файлами которые Git уже отслеживает и игнорирует все остальные файлы!**
