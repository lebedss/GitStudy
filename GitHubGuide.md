...
Вместо user-name подставьте логин, который указывали при регистрации. В случае на скориншоте это test-github-04, а вместо email@example.com — адрес вашей электронной почты. В данном примере — testgithub@gmail.com.
...
Отправляем все изменения на гитхабовский сервер. Если мы зайдем на страницу репозитория на гитхабе, то увидим свой проект.
...
*git remote add upstream <скопированный-адрес-upstream>* (скопированный-адрес-upstream это адресс оригинального репозитория от которого мы делали ответвление, а не сам форк) 
...
Также имеет значение нейминг. Будет любезно с вашей стороны использовать имя, которое легко сможет понять человек, ничего не знающий об этом репозитории. Например, если вы хотите добавить возможность логиниться в приложении, можно создать ветку под названием new_branch_for_login_feature
...
Шаг 8: [открываем пул-реквест](Project_Images/PullRequest.png) на GitHub
...
Именно благодаря наличию сиcтемы коментариев отсеивается огромное количество конфликтов и багов...
[Выглядит это так](Project_Images/MergePullRequest.png)

**Заключение о cоздании ответвлений (fork):** 
