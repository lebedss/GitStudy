...
Наш локальный проект на компьютере  может размещатся в любом каталоге. Для него нужно инициализировать git. 
...
Отправляем все изменения на гитхабовский сервер. Если мы зайдем на страницу репозитория на гитхабе, то также увидим свой проект.
...
* Шаг 1-ый: для того, чтобы создать ответвление проекта, зайдите на страницу проекта и нажмите кнопку «Создать ответвление» («Fork»),[тыкаем кнопку](Project_Images\forkbutton.png) которая расположена в правом верхнем углу.

* Шаг 2-ой: [клонируйте проект на свою машину](Project_Images\CloneYourProjectToPC.png)
 Перейдите в свой форк репозитория (который теперь находится в ваших репозиториях GitHub). Затем клонируйте проект на свою машину, введя в окне вашего терминала:*git clone <скопированный-адрес>*
Переходим в клонированную директорию: cd [NAME OF REPOSITORY]
...
Можно выполнить команду *git pull upstream master* чтобы убедиться, не произошло ли каких-то изменений (с того момента как вы сделали форк и до настоящей минуты).
Имейте в виду, что upstream это оригинальный репозиторий (в примере – репозиторий freeCodeCamp), а не форк. 

* Шаг 4-ый: создание ветки (branch), над которой вы хотите поработать
...
Также имеет значение нейминг. Будет любезно с вашей стороны использовать имя, которое легко сможет понять человек, ничего не знающий об этом репозитории. Например, если вы хотите добавить возможность логиниться в приложении, можно создать ветку под названием  
new_branch Для создания ветки введите в терминале следующую команду:
*git checkout -b new_branch* Будет создана ветка и осуществлен переход в нее.
...
Все почти готово. «Пушим» изменения в свою рабочую ветку:*git push origin <имя-ветки>*

Шаг 8: [открываем пул-реквест](Project_Images\PullRequest.png)
...
Вы открываете пул-реквест, и если собственнику репозитория или людям, занимающимся его поддержкой, ваши изменения понравятся, они сольют их со своим проектом (merge). Также они могут внести какие-то изменения, а уж затем выполнить слияние, или запросить эти изменения.Для открытия пул-реквеста перейдите в ваш форк репозитория, как показано ниже. Вы увидите последнюю ветку, в которую вы делали push. Кликните на «compare and pull request». [Четко поясните, что именно вы изменили, и создайте пул-реквест](Project_Images\CreatePullRequest.png)

[Создание коментариев для pull request](Project_Images\CommentsForPullRequest.png)

[Полученный коментарий через pull request](Project_Images\CommentsForPullRequestsWereSent.png)

В конечном счёте владелец проекта которому прислали pull request может либо принять предлагаемые изменения - confirm merge
либо же оказать - close pull request. Именно благодаря ситеме коментариев отсеивается большинство конфликтов и багов...
